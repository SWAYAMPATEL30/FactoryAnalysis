  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
      {ORDER.map((key) => {
        ...
        return (
          <div key={key} className="rounded-md border border-line bg-raised p-2.5">
            <div className="mb-1.5 font-mono text-[10px] font-semibold uppercase tracking-wide text-ink-faint">{key}</div>
            <div className="mb-1.5 h-[4px] overflow-hidden rounded-full bg-raised-2">
              <div
                className="h-full rounded-full transition-[width] duration-500"
                style={{ width: `${pct}%`, background: COLOR[key] }}
              />
            </div>
            <div className="font-mono text-base font-bold text-ink">{sums[key].toFixed(1)}s</div>
            <div className="font-mono text-[10px] text-ink-faint">{Math.round(pct)}%</div>
          </div>
        );
      })}
    </div>
